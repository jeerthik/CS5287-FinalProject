...
def send_request(server, outer_iters, inner_iters, results_file, all_ids):
    try:
        ...
        for j in range(outer_iters):
            ...
            avg_resp_times.append(resp_time_this_iter / inner_iters)
        return all_resp_times, avg_resp_times
...
def main():
    ...
    try:
        ...
        all_resp_times, avg_resp_times = send_request(server, parsed_args.outer_iters, parsed_args.inner_iters, results_file, all_ids)
        
        total_resp_time = np.sum(all_resp_times)
        avg_resp_time = np.mean(all_resp_times)
        std_resp_time = np.std(all_resp_times)
        results_file.write(f"Total Number of Iterations: {parsed_args.inner_iters * parsed_args.outer_iters}, Total Response TIme: {total_resp_time}, Mean Response Time: {avg_resp_time}, STD DEV Response Time: {std_resp_time}")
        results_file.write("Outer iters average response times:\n")
        results_file.write(",".join([str(avg_time) for avg_time in avg_resp_times]))
        print(f"Total Number of Iterations: {parsed_args.inner_iters * parsed_args.outer_iters}, Total Response TIme: {total_resp_time}, Mean Response Time: {avg_resp_time}, STD DEV Response Time: {std_resp_time}")
        print(all_resp_times)
        # close the file 
